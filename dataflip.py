...
driver = webdriver.Chrome(executable_path="/opt/render/project/.render/chrome/opt/google/chrome/google-chrome/chromedriver",options=options,service=service)
...
def prevedi_2(list_sentences):
    ...
    for sentence in list_sentences:
        try:
            prijevod=translator.translate(sentence, src='en', dest='hr')
...
def prevedi_selenium(tekst):
    LOCAL_LIST.clear()
    #KINESKI=[]
    #for kineska_recenica in re.findall(u'[^!?。\.\!\?]+[!?。\.\!\?]?', tekst, flags=re.U):
    #    print(kineska_recenica)
    #    KINESKI.append(kineska_recenica)
    #    print('------------')
...
def process(ticker):
    ...
    TEXT=[]
    duzina=0
    #nltk_sentences = nltk.sent_tokenize()
    print('--------------------------------------------')
    print('--------------------------------------------')
